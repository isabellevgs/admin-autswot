...
export async function buscarUsuariosPorEmails(emails) {
  try {
    const res = await api.post('/users/by-emails', { emails });
    return { usuarios: res.data.usuarios, erro: null };
  } catch (err) {
    return {
      usuarios: null,
      erro: extrairErroApi(err, 'Erro ao buscar usuários.'),
    };
  }
}
